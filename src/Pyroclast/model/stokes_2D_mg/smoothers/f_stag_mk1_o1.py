...
File: Pyroclast/model/stokes_2D_mg/smoothers/f_stag_mk1_o1.py 
                ...
                for j in range(1, nx1 - 2 + 3):
                    for i in range(start_b, end_b):
                        ...
                        if (i + j - 1) % 2 == 1:
                            if 1 <= j-1 < nx1 - 2:
                                # Black pass vx
                                vx[i, j - 1] = inline_loop_body_vx(i=i, j=j - 1, dx=dx, dy=dy, relax_v=relax_v,
                                                                   etap=etap, etab=etab,
                                                                   vx=vx, vy=vy, rhs=vx_rhs)
                            if 1 <= j-3 < nx1 - 2:
                                # Black pass vy
