...
"""
Attempt 8: Reimplement the fully staggered loop with correct staggering. And Thread Blocking. And Boundary Jitter
"""
                    ...
                    for i in range(1, ny1 - 1 + step_size * 3):
                        for j in range(start_bx, end_bx):
                            ...
                            if (iloc2 + j) % 2 == 0 and 1 <= j <= nx1 - 1 and 1 <= iloc2 <= ny1 - 2:
                                    vy[iloc2, j] = inline_loop_body_vy(i=iloc2, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                                                       etap=etap, etab=etab,
                            ...
                            if (iloc3 + j) % 2 == 0 and 1 <= j <= nx1 - 1 and 1 <= iloc3 <= ny1 - 2:
                                    vy[iloc3, j] = inline_loop_body_vy(i=iloc3, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                                                       etap=etap, etab=etab,
                                                                       vx=vx, vy=vy, rhs=vy_rhs)
