...
File: Pyroclast/model/stokes_2D_mg/smoothers/f_stag_mk1_o2.py 
Description: Internal If implementation of staggered red-black gauss-seidel, offset is 2, and fully staggered 
    ...
    for _ in range(max_iter):
        ...
        for p in nb.prange(th):
            ...
            for b in range(blocks):
                ...
                # Iterate through j, add + 6 for offset for second pass, and vy pass
                for j in range(1, nx1 - 2 + 6):
                    for i in range(start_b, end_b):
                        ...
                        if (i + j) % 2 == 0:
                            ...
                            if 1 <= j-4 < nx1 - 2:
                                # Red pass vy
                                vy[i, j-4] = cpu_inline_loop_body_vy(i=i, j=j - 4, dx=dx, dy=dy, relax_v=relax_v,
                                                                     etap=etap, etab=etab,
                        ...
                        if (i + j - 2) % 2 == 1:
                            ...
                            if 1 <= j-6 < nx1 - 2:
                                # Black pass vy
                                vy[i, j - 6] = cpu_inline_loop_body_vy(i=i, j=j - 6, dx=dx, dy=dy, relax_v=relax_v,
                                                                       etap=etap, etab=etab,
                                                                       vx=vx, vy=vy, rhs=vy_rhs)
