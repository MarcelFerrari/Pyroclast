...
vx_op_point_device = cuda.jit(device=True, inline="always")(vx_op_point)
vy_op_point_device = cuda.jit(device=True, inline="always")(vy_op_point)
p_op_point_device  = cuda.jit(device=True, inline="always")(p_op_point)
uzawa_vx_op_point_device = cuda.jit(device=True, inline="always")(uzawa_vx_op_point)
uzawa_vy_op_point_device = cuda.jit(device=True, inline="always")(uzawa_vy_op_point)
uzawa_vx_rhs_point_device = cuda.jit(device=True, inline="always")(uzawa_vx_rhs_point)
uzawa_vy_rhs_point_device = cuda.jit(device=True, inline="always")(uzawa_vy_rhs_point)
p_energy_point_device = cuda.jit(device=True, inline="always")(p_energy_point)
vx_energy_point_device = cuda.jit(device=True, inline="always")(vx_energy_point)
vy_energy_point_device = cuda.jit(device=True, inline="always")(vy_energy_point)
...
def _vx_operator_kernel(dx, dy, etap, etab, vx, vy, p, out, nx1, ny1):
    i, j = cuda.grid(2)  # i=row (y), j=col (x)
    # interior: i in [1, ny1-2], j in [1, nx1-3]
    if i >= 1 and i <= ny1 - 2 and j >= 1 and j <= nx1 - 3:
...
def _vy_operator_kernel(dx, dy, etap, etab, vx, vy, p, out, nx1, ny1):
    ...
    if i >= 1 and i <= ny1 - 3 and j >= 1 and j <= nx1 - 2:
...
def _p_operator_kernel(dx, dy, vx, vy, out, nx1, ny1):
    ...
    if i >= 1 and i <= ny1 - 2 and j >= 1 and j <= nx1 - 2:
...
def compute_p_energy_norm(nx1, ny1, dx, dy, etap, p_res):
    ...
    cuda.synchronize()
    total = cp.sum(p_res)                 # on device
    return float(cp.sqrt(total).item())   # returns Python float
...
def compute_vx_energy_norm(nx1, ny1, dx, dy, etap, etab, vx_res):
    ...
    cuda.synchronize()
    total = cp.sum(vx_res)
    return float(cp.sqrt(total).item())
