...
def rb_gs_red_vx(vx: DeviceNDArray, vy: DeviceNDArray,
    ...
    if (i + j) % 2 == 0 and 1 <= j <= nx1 - 2 and 1 <= i <= ny1 - 1:
        vx[i, j] = gpu_inline_loop_body_vx(i=i, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                           etap=etap, etab=etab,
                                           vx=vx, vy=vy, rhs=vx_rhs)


@cuda.jit
def rb_gs_black_vx(vx: DeviceNDArray, vy: DeviceNDArray,
                   etap: DeviceNDArray, etab: DeviceNDArray, vx_rhs: DeviceNDArray,
                   dx: float, dy: float, relax_v: float, nx1: int, ny1: int):
    """
    Cuda Kernel for Black Pass in VX
    """
    j = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    i = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y

    if (i + j) % 2 == 1 and 1 <= j <= nx1 - 2 and 1 <= i <= ny1 - 1:
        vx[i, j] = gpu_inline_loop_body_vx(i=i, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                           etap=etap, etab=etab,
                                           vx=vx, vy=vy, rhs=vx_rhs)

@cuda.jit
def rb_gs_red_vy(vx: DeviceNDArray, vy: DeviceNDArray,
                 etap: DeviceNDArray, etab: DeviceNDArray, vy_rhs: DeviceNDArray,
                 dx: float, dy: float, relax_v: float, nx1: int, ny1: int):
    """
    Cuda Kernel for Red Pass in VY
    """
    j = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    i = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y

    if (i + j) % 2 == 0 and 1 <= j <= nx1 - 1 and 1 <= i <= ny1 - 2:
        vy[i, j] = gpu_inline_loop_body_vy(i=i, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                           etap=etap, etab=etab,
                                           vx=vx, vy=vy, rhs=vy_rhs)

@cuda.jit
def rb_gs_black_vy(vx: DeviceNDArray, vy: DeviceNDArray,
                   etap: DeviceNDArray, etab: DeviceNDArray, vy_rhs: DeviceNDArray,
                   dx: float, dy: float, relax_v: float, nx1: int, ny1: int):
    """
    Cuda Kernel for Black Pass in VY
    """
    j = cuda.blockIdx.x * cuda.blockDim.x + cuda.threadIdx.x
    i = cuda.blockIdx.y * cuda.blockDim.y + cuda.threadIdx.y

    if (i + j) % 2 == 1 and 1 <= j <= nx1 - 1 and 1 <= i <= ny1 - 2:
        vy[i, j] = gpu_inline_loop_body_vy(i=i, j=j, dx=dx, dy=dy, relax_v=relax_v,
                                           etap=etap, etab=etab,
                                           vx=vx, vy=vy, rhs=vy_rhs)
...
def velocity_smoother_rb_gs_cuda(
    ...
    for it in range(max_iter):
        rb_gs_red_vx[grid_dim, threadsperblock](
            vx_d, vy_d,
            etap_d, etab_d, vx_rhs_d,
            dx, dy, relax_v, nx1, ny1)
        rb_gs_black_vx[grid_dim, threadsperblock](
            vx_d, vy_d,
            etap_d, etab_d, vx_rhs_d,
            dx, dy, relax_v, nx1, ny1)
        rb_gs_red_vy[grid_dim, threadsperblock](
            vx_d, vy_d,
            etap_d, etab_d, vy_rhs_d,
            dx, dy, relax_v, nx1, ny1)
        rb_gs_black_vy[grid_dim, threadsperblock](
            vx_d, vy_d,
            etap_d, etab_d, vy_rhs_d,
            dx, dy, relax_v, nx1, ny1)
